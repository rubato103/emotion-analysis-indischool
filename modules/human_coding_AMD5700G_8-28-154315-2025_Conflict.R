  ...
  tryCatch({
    ...
    reference_data <- data.frame(
      ...
      설명 = c(
        ...
        "태블릿: 열 선택 → 도구 모음 → 체크박스",
        "기쁨~기대: AI가 분석한 8개 감정별 점수 (0.00~1.00)",
...
move_sheets_to_folder <- function(sheet_urls, sample_label, timestamp) {
  ...
  tryCatch({
    ...
    log_message("INFO", sprintf("폴더 생성/이동 시도: %s", folder_name))
    
    # 폴더 검색 또는 생성 (권한 확인 포함)
    ...
    if (nrow(folder_info) == 0) {
      ...
      }, error = function(folder_error) {
        log_message("WARN", sprintf("폴더 생성 실패: %s", folder_error$message))
        if (grepl("403|Forbidden|PERMISSION_DENIED", folder_error$message)) {
          log_message("INFO", "💡 폴더 생성 권한 부족 - 시트는 루트 폴더에 그대로 둡니다")
          log_message("INFO", "📁 수동으로 폴더를 생성한 후 시트를 이동하세요")
        }
        return(NULL)
      })
    }
    
    # 폴더가 성공적으로 생성되었거나 존재하는 경우에만 이동 시도
    if (!is.null(folder_info) && nrow(folder_info) > 0) {
      log_message("INFO", "시트를 폴더로 이동 중...")
      ...
      for (coder_name in names(sheet_urls)) {
        ...
        if (!is.null(sheet_id)) {
          ...
          }, error = function(move_error) {
            log_message("WARN", sprintf("❌ %s 시트 이동 실패: %s", coder_name, move_error$message))
            
            # 403 Forbidden 오류 처리
            if (grepl("403|Forbidden|PERMISSION_DENIED", move_error$message)) {
              log_message("INFO", sprintf("💡 %s 시트 이동 권한 부족", coder_name))
              log_message("INFO", "   - 시트는 구글 드라이브 루트에 생성되었습니다")
              log_message("INFO", "   - 필요시 수동으로 폴더로 이동해주세요")
            } else if (grepl("404|Not Found", move_error$message)) {
              log_message("WARN", sprintf("💡 %s 시트를 찾을 수 없습니다", coder_name))
            } else {
              log_message("WARN", sprintf("💡 %s 예상치 못한 이동 오류", coder_name))
            }
          })
        } else {
          log_message("WARN", sprintf("❌ %s 시트 ID를 추출할 수 없습니다", coder_name))
        }
      }
    } else {
      log_message("INFO", "📁 폴더 생성/검색 실패로 시트는 루트 폴더에 유지됩니다")
      log_message("INFO", "   시트 생성은 성공적으로 완료되었으니 URL을 통해 접근하세요")
    }
    
  }, error = function(e) {
    log_message("WARN", sprintf("폴더 작업 중 전체 오류: %s", e$message))
    log_message("INFO", "📝 시트 생성 자체는 성공했으므로 URL을 통해 접근 가능합니다")
  })
}
